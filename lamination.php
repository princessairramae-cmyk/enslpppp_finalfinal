...
$date_laminated= $_POST['date_laminated'];
...
/* CHECK IF ADHESIVE IS COMPLETED */

$checkAdhesive = $conn->prepare("
SELECT *
FROM adhesive_jobs
WHERE work_order_id=?
...
$checkAdhesive->execute([$work_order_id]);

$adhesiveDone = $checkAdhesive->fetch(PDO::FETCH_ASSOC);

if(!$adhesiveDone){

$_SESSION['error'] =
"Cannot proceed to lamination. Adhesive must be completed first.";
...
(work_order_id,item_id,material,operator,date_laminated,quantity,status)
VALUES (?,?,?,?,?,?,'Pending')
");
...
data-bs-target="#addLaminationModal">
...
<td>
<?=date('Y-m-d', strtotime($row['date_laminated']))?>
</td>
...
<a href="update_lamination_status.php?id=<?=$row['id']?>&status=On Process"
...
<a href="update_lamination_status.php?id=<?=$row['id']?>&status=Completed"
...
<div class="modal fade" id="addLaminationModal">
...
fetch(
'get_lamination_material.php?work_order_id='
+ wo_id
)
